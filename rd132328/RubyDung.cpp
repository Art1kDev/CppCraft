...
void RubyDung::init() {
    ...
    for (int i = 0; i < 100; ++i) {
        float x = 128.0f + (float)((std::rand() % 120) - 60);
        float z = 128.0f + (float)((std::rand() % 120) - 60);
        float y = (float)(level.depth + 10);
        zombies.push_back(new Zombie(&level, x, y, z));
    }
}
...
void RubyDung::render(float a) {
    ...
    if (g && !gPressed) {
        float distance = 3.0f;
        float spawnX = player.x + sinf(player.yRot * 3.14159f / 180.0f) * distance;
        float spawnZ = player.z - cosf(player.yRot * 3.14159f / 180.0f) * distance;
        float spawnY = player.y + 0.5f;
        zombies.push_back(new Zombie(&level, spawnX, spawnY, spawnZ));
    }
    gPressed = g;
