...
Player::Player(Level* level) : level(level), onGround(false), yRot(0), xRot(0), xd(0), yd(0), zd(0), bb(0,0,0,0,0,0) {
    resetPos();
}
...
void Player::tick() {
    ...
    if (glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_UP) == GLFW_PRESS || 
        glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_W) == GLFW_PRESS) ya--;
    if (glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_DOWN) == GLFW_PRESS || 
        glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_S) == GLFW_PRESS) ya++;
    if (glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_LEFT) == GLFW_PRESS || 
        glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_A) == GLFW_PRESS) xa--;
    if (glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_RIGHT) == GLFW_PRESS || 
        glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_D) == GLFW_PRESS) xa++;
